(function (factory) {
    ...
    function worker () {
      ...
      var solveCaptcha = function solveCaptcha(data, nonce) {
        if (nonce === void 0) {
          nonce = 1;
        }
    ...
    (function () {
      var script = document.getElementById("whcScriptTag");
      var forms = Array.from(document.getElementsByClassName(script.dataset.form));

      var Constructor = function Constructor(form, index) {
        var Private = {}; // now converted to seconds
        ...
        if (Private.debug) {
          ...
          window.addEventListener(Private.eventName, function (_ref) {
            var detail = _ref.detail;
            return console.log(Private.eventName + detail);
          }, false);
        }

        var emit = function emit(detail) {
